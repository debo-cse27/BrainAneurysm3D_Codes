...
if __name__ == "__main__": 
    ...
    model.train(total_time = 0.55, learning_rate=1e-3)
    #model.train(total_time = 0.04, learning_rate=1e-3) #For 10 epochs
    ...
    for snap in range(0,t_star.shape[0]):
        ...
        grid.save("../Results/Aneurysm3D/checkvtktwo/allresult_%s.vtk" %snap)
    ...
    scipy.io.savemat("../Results/Aneurysm3D/%s.mat" %(time.strftime("%m_%d_%Y_%H_%M_%S")),
                     {'C_pred':C_pred, 'U_pred':U_pred, 'V_pred':V_pred, 'W_pred':W_pred, 'P_pred':P_pred})

    # gridToVTK(
    # '../Results/Aneurysm3D_Wall_Stresses_results_%s' %(time.strftime('%d_%m_%Y')), 
    # t_test, x_test, y_test, z_test,  # Coordinates
    # pointData={'C_pred':C_pred, 'U_pred':U_pred, 'V_pred':V_pred, 'W_pred':W_pred, 'P_pred':P_pred}  # Point data
    # )